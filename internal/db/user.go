...
// Postgres does not support LastInsertId. Use Insert and return the ID by selecting it from the database.
...
`
	var id int
	err := db.QueryRow(query, UserName, Email, Phone, AvatarUrl, Role, PasswordHash, DateOfBirth).Scan(&id)
	if err != nil {
		log.Println(err)
		return 0, fmt.Errorf("failed to sign up: %v", err)
	}
...
func Login(db *sql.DB, email, password string) (*UserDataLoginResp, error) {
	...
	const query = `SELECT id, COALESCE(username, ''), COALESCE(description, ''), email, phone, avatar, COALESCE(status, ''), 
	...
	err = db.QueryRow(query, email, checkedPassword).Scan(
		&userDataLogin.ID, &userDataLogin.UserName, &userDataLogin.Description, &userDataLogin.Email, &userDataLogin.Phone, &userDataLogin.AvatarUrl,
		&userDataLogin.Status, &userDataLogin.Role, &userDataLogin.DateOfBirth, &userDataLogin.PrivacySettings, &userDataLogin.IsActive,
